...
PDF_SAVE_DIR = os.path.join(Path.home(), "Desktop", "注文書") # 保存先をデスクトップの「注文書」フォルダに
...
def send_smtp_mail(info, pdf_path):
    try:
        msg = MIMEMultipart()
...
class Application(ttk.Frame):
    ...
    def start_data_retrieval(self):
        ...
        if not all([NOTION_API_TOKEN, EMAIL_SENDER, EMAIL_PASSWORD]): return messagebox.showerror("設定エラー", ".envファイルにNotionとEmailの設定が必要です。")
    ...
    def send_mail_task(self):
        selected_supplier = self.supplier_listbox.get(self.supplier_listbox.curselection())
        print(f"「{selected_supplier}」宛にメールを送信中...")
        items = [item for item in self.order_data if item["supplier_name"] == selected_supplier]
        success = send_smtp_mail(items[0], self.current_pdf_path)
        if success:
    ...
    def update_data_ui(self, data):
        ...
        if not data:
            ...
            self.log("-> データ取得完了。右のリストから仕入先を選択してください。")
    ...
    def update_preview_ui(self, data):
        ...
        self.log("   宛先、担当者、PDFの内容を必ず確認してください。")
        self.log("   (PDFファイル名をクリックすると内容を開けます)")
        self.log("-> 問題がなければ「メール送信」ボタンを押してください。")
    ...
    def mark_as_sent(self, supplier, updated=True):
        ...
        try:
            ...
            self.supplier_listbox.itemconfig(idx, {'fg': 'gray'})
            self.supplier_listbox.selection_clear(idx)
